(function($) {
  ...
  $.fn.fireworks = function(options) {
    if(options === 'stop'){
      clearInterval($(this).data('launchTimeid'));
      clearInterval($(this).data('loopTimeid'));
      audio.pause();
      $(this).empty();
      return;
    }
    ...
    var launch = function() {
      launchFrom(SCREEN_WIDTH / 8);
      launchFrom(SCREEN_WIDTH / 1.2);
    };

    // Append the canvas and start the loops
    $(fireworksField).append(canvas);
    $(fireworksField).data('launchTimeid', setInterval(launch, 800));
    $(fireworksField).data('loopTimeid', setInterval(loop, 1000 / 100));
    if(options.sound){
      audio.currentTime = 0;
      setTimeout(function(){
        audio.play();
